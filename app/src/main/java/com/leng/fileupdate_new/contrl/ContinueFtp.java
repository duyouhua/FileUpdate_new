...
public class ContinueFtp {
	...
	public ContinueFtp(Context context, Handler handler) {
	...
	public boolean connect() throws IOException {
		...
		try {
			...
			Log.i(TAG, "ftp connect failed " + e);
			return false;
		} catch (IOException e) {
	 ...
	 * @throws IOException
	 */
	public UploadStatus upload(String PATH) throws IOException {
		UploadStatus result;
		// 对远程目录的处理
		String local = PATH;
...
//			remotePath = "/Other/";
		...
		String oldFileName = new String(PATH.getBytes(
				LOCAL_CHARSET), SERVER_CHARSET);
		String httpOldFileName = new String(PATH.getBytes(
				"UTF-8"), "UTF-8");
...
//			remoteFileName = Utils.getRemoteFileName(remotePath,httpOldFileName,local);
			...
			if (CreateDirecroty(remote, ftpClient) == UploadStatus.Create_Directory_Fail) {
				 Log.i(TAG, "创建远程目录穿件远程目录跟这个没关系");
				return UploadStatus.Create_Directory_Fail;
			}
			Log.i(TAG, "创建远程目录穿件远程目录2");
		}
		...
		Log.i(TAG, "这个长度值是什么意思   还不知"+files.length);
		if (files.length == 1) {
...
//				Log.i(TAG, "file name is " + mBxFile.getFileName()
...
//			}
		Log.i(TAG,"断点续传等于一是什么意思"+files.length);
		} else {
			Log.i(TAG,"断点续传没有等于一是什么意思"+files.length+"======"+remoteFileName);
			result = uploadFile(remoteFileName, new File(local), ftpClient, 0);
		}
