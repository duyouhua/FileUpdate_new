...
//枚举类UploadStatus代码
    ...
    private final static String TAG="ContinueFTP2";
     ...
     * @throws IOException
     */
    public ContinueFTP2(Context context){
        mContext=context;
     ...
     * @return 上传结果
     * @param type  1image  2audio  3video
     * @throws IOException
     */
    public UploadStatus upload(String local,String remote,String type) throws IOException{
        //设置PassiveMode传输
        ...
        if(remote.contains("/")){
            ...
            Log.i("QWEQWE","Q"+remote);
            if(CreateDirecroty2(remote, ftpClient)==UploadStatus.Create_Directory_Fail){
        ...
        if(files.length == 1){
            ...
            Log.i(TAG,"本地路径："+local+" 文件类型"+type);
            result = uploadFile(remoteFileName, new File(local), ftpClient, 0);
            httpUtils2 mHttpUtils = new httpUtils2(mContext, type,
                    local, remoteFileName);
            mHttpUtils.getHttpRequest();
            mHttpUtils.getHttpResult();
            Log.i(TAG,"本地路径："+local+" 文件类型"+type);
        }
        return result;
    }
    ...
    public UploadStatus CreateDirecroty2(String remote, FTPClient ftpClient)
            throws IOException {
